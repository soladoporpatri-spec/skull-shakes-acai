# -*- coding: utf-8 -*-
import re

content = '''import { CheckoutState } from '@/hooks/useCheckout';

type Props = {
  payment: CheckoutState['payment'];
  updatePayment: (fields: Partial<CheckoutState['payment']>) => void;
  errors: Record<string, string>;
};

const METHODS: { label: string; value: string }[] = [
  { label: 'PIX',               value: 'Pix'           },
  { label: 'Cartão de Crédito', value: 'CreditCard'    },
  { label: 'Cartão de Débito',  value: 'DebitCard'     },
  { label: 'Dinheiro',          value: 'Cash' },
];

export default function PaymentMethod({ payment, updatePayment, errors }: Props) {
  // Se for dinheiro, força a modalidade para OnDelivery
  const handleMethodSelect = (value: string) => {
    if (value === 'Cash' || value === 'DebitCard') {
      updatePayment({ method: value, modality: 'OnDelivery', needsChange: false, changeFor: '' });
    } else {
      updatePayment({ method: value, needsChange: false, changeFor: '' });
    }
  };

  return (
    <section className="mb-16">
      <h3 className="text-2xl font-bold uppercase mb-2 text-white flex justify-between items-center">
        Forma de Pagamento
        {errors.method && <span className="text-red-400 text-xs tracking-widest font-normal">{errors.method}</span>}
      </h3>
      <p className="text-zinc-400 mb-6 font-light text-sm">Escolha como deseja pagar seu pedido.</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {METHODS.map(({ label, value }) => (
          <button
            key={value}
            type="button"
            onClick={() => handleMethodSelect(value)}
            className={p-5 border text-left transition-all uppercase text-sm font-bold tracking-wider }
          >
            {label}
          </button>
        ))}
      </div>

      {(payment.method === 'Pix' || payment.method === 'CreditCard') && (
        <div className="mb-8">
          <label className="text-xs uppercase tracking-widest text-zinc-400 mb-4 block font-bold">Onde deseja pagar?</label>
          <div className="flex gap-4">
            <button 
              type="button" 
              onClick={() => updatePayment({ modality: 'Online' })} 
              className={px-6 py-4 border text-sm font-bold uppercase transition-colors flex-1 }
            >
              Pagar agora no site
            </button>
            <button 
              type="button" 
              onClick={() => updatePayment({ modality: 'OnDelivery' })} 
              className={px-6 py-4 border text-sm font-bold uppercase transition-colors flex-1 }
            >
              Pagar na entrega
            </button>
          </div>
        </div>
      )}

      {payment.method === 'Cash' && (
        <div className="mt-6 p-6 border border-white/10 bg-white/5">
          <label className="text-xs uppercase tracking-widest text-zinc-400 mb-4 block font-bold">Precisa de troco?</label>
          <div className="flex gap-4 mb-6">
            <button type="button" onClick={() => updatePayment({needsChange: false, changeFor: ''})} className={px-6 py-3 border text-sm font-bold uppercase transition-colors }>Não</button>
            <button type="button" onClick={() => updatePayment({needsChange: true})} className={px-6 py-3 border text-sm font-bold uppercase transition-colors }>Sim</button>
          </div>

          {payment.needsChange && (
            <div className="flex flex-col mt-6">
              <label htmlFor="changeFor" className="text-xs uppercase tracking-widest text-zinc-400 mb-2 font-bold flex justify-between">
                Troco para quanto?
                {errors.changeFor && <span className="text-red-400 font-normal">{errors.changeFor}</span>}
              </label>
              <input
                id="changeFor"
                type="text"
                inputMode="numeric"
                value={payment.changeFor}
                onChange={(e) => updatePayment({ changeFor: e.target.value })}
                placeholder="Ex: 50"
                className={g-transparent border-b  outline-none py-3 px-2 text-white placeholder:text-zinc-700 transition-colors rounded-none w-full max-w-xs focus:bg-white/5}
              />
            </div>
          )}
        </div>
      )}
    </section>
  );
}
'''
with open('web/src/components/checkout/PaymentMethod.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
